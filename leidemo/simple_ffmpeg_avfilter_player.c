...
const char *filterdescr = "movie=/Users/apple/Desktop/MV/大鱼/周深 - 大鱼_72.bmp[wm];[in][wm]overlay=5:5[out]";
...
int mainsimple_avfilter(int argc, char* argv[])
{
    ...
    if ((ret = open_input_file(fileName)) < 0)
        goto end;
    if ((ret = init_filters(filterdescr)) < 0)
        goto end;
    ...
    int screen_width=pCodecCtx->width;
    int screen_height=pCodecCtx->height;
    ...
    window=SDL_CreateWindow(pFormatCtx->filename, SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,screen_width , screen_height, SDL_WINDOW_OPENGL);
        ...
        if (packet.stream_index == video_stream_index) {
            ...
            if (got_frame) {
                ...
                while (1) {
                    ...
                    printf("Process 1 frame! %d %d\n",pFrame_out->format,AV_PIX_FMT_YUV420P);
                    
                    if (pFrame_out->format==AV_PIX_FMT_YUVA420P) {
#if ENABLE_YUVFILE
                        //Y, U, V
...
//                        //Delay 40ms
                        SDL_UpdateYUVTexture(texture, &videoRect, pFrame_out->data[0],pFrame_out->linesize[0],pFrame_out->data[1],pFrame_out->linesize[1]
